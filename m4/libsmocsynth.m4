...
dnl ACJF_CHECK_LIB_SYNTHESIS_MULTICORE check for Synthesis MULTICORE
dnl
dnl ACJF_CHECK_LIB_SYNTHESIS_MULTICORE(
dnl  [<tags>,]
dnl  [<code if found, default does nothing>,
dnl  [<code if not found, default is bailout>]])
AC_DEFUN([ACJF_CHECK_LIB_SYNTHESIS_MULTICORE], [ACJF_CHECK_HELPER_SET_VARS([$@], [
  ACJF_ARG_WITHPKG([Synthesis-MultiCore], ACJF_TAGS_OVERRIDE(ACJF_VAR_TAGS,[[intern],[extern],[pkgconfig:libsmocsynth-cxx-multicore]]))dnl
  AC_LANG_PUSH([C++])
  ACJF_CHECK_LIB_TESTER([Synthesis-MultiCore], ACJF_TAGS_OVERRIDE(ACJF_VAR_TAGS,[[intern],[pkgconfig:libsmocsynth-cxx-multicore]]),
    [_ACJF_CHECK_LIB_SYNTHESIS_XXX_TESTMACRO],
    m4_if(ACJF_VAR_CODE_IF_TRUE[]ACJF_VAR_CODE_IF_FALSE, [], [], [[true;]]))dnl
  [pkg_synthesis_multicore_pkgdatadir="${acjf_cv_synthesis_multicore_pkgdatadir}"]
  [pkg_synthesis_multicore_bindir="${acjf_cv_synthesis_multicore_bindir}"]
  AC_SUBST([pkg_synthesis_multicore_pkgdatadir])
  AC_SUBST([pkg_synthesis_multicore_bindir])
  if test [x"${SYNTHESIS_MULTICORE_FOUND}" = x"yes";] then
    m4_if(ACJF_VAR_CODE_IF_TRUE, [],
     [true;],
     [dnl echo "ACJF_VAR_CODE_IF_TRUE";
      ACJF_VAR_CODE_IF_TRUE])
